...
public class CachedBufferAllocator implements IoBufferAllocator {
   ...
   * 
   * @param maxPoolSize the maximum number of buffers with the same capacity per thread. <tt>0</tt>
   *        disables this limitation.
   * @param maxCachedBufferSize the maximum capacity of a cached buffer. A buffer whose capacity is
   *        bigger than this value is not pooled. <tt>0</tt> disables this limitation.
   */
  ...
  /**
   * Returns the maximum number of buffers with the same capacity per thread. <tt>0</tt> means 'no
   * limitation'.
   */
