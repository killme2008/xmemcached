 ...
 * <h2>AutoExpand</h2>
 * <p>
 * Writing variable-length data using NIO <tt>ByteBuffers</tt> is not really easy, and it is because
 * its size is fixed. {@link IoBuffer} introduces <tt>autoExpand</tt> property. If
 * <tt>autoExpand</tt> property is true, you never get {@link BufferOverflowException} or
 ...
 * is not being used. {@link IoBuffer} provides <tt>autoShrink</tt> property to take care of this
 * issue. If <tt>autoShrink</tt> is turned on, {@link IoBuffer} halves the capacity of the buffer
 ...
 * or {@link #setAutoShrink(boolean)} with <tt>true</tt> parameter will raise an
...
public abstract class IoBuffer implements Comparable<IoBuffer> {
  ...
  /**
   * returns <tt>true</tt> if and only if this buffer is derived from other buffer via
   ...
   * <tt>expectedRemaining</tt> room from the current position. This method works even if you didn't
   * set <tt>autoExpand</tt> to <tt>true</tt>.
   */
  public abstract IoBuffer expand(int expectedRemaining);

  /**
   * Changes the capacity and limit of this buffer so this buffer get the specified
   * <tt>expectedRemaining</tt> room from the specified <tt>position</tt>. This method works even if
   * you didn't set <tt>autoExpand</tt> to <tt>true</tt>.
   */
   ...
   * returns <tt>-1</tt> if the buffer position reaches to the limit.
   */
  ...
  /**
   * Writes the content of <code>in</code> into this buffer as a <code>NUL</code>-terminated string
   * using the specified <code>encoder</code>.
   * <p>
   ...
   * is a shortcut for <tt>putPrefixedString(in, 2, 0, encoder)</tt>.
   * 
   * @throws BufferOverflowException if the specified string doesn't fit
   */
   ...
   * Writes the content of <code>in</code> into this buffer as a string which has a 16-bit length
   * field before the actual encoded string, using the specified <code>encoder</code>. This method
   * is a shortcut for <tt>putPrefixedString(in, prefixLength, 0, encoder)</tt>.
   * 
   * @param prefixLength the length of the length field (1, 2, or 4)
   * 
   * @throws BufferOverflowException if the specified string doesn't fit
   ...
   * Writes the content of <code>in</code> into this buffer as a string which has a 16-bit length
   * field before the actual encoded string, using the specified <code>encoder</code>. This method
   * is a shortcut for <tt>putPrefixedString(in, prefixLength, padding, ( byte ) 0, encoder)</tt> .
   * 
   * @param prefixLength the length of the length field (1, 2, or 4)
