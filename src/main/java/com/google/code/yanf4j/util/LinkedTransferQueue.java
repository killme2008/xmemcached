...
public class LinkedTransferQueue<E> extends AbstractQueue<E>
	...
	private static final class QNode extends AtomicReference<Object> {
		...
		static {
			...
			try {
				tmp = AtomicReferenceFieldUpdater.newUpdater(QNode.class,
						QNode.class, "next");
	...
	public static final class PaddedAtomicReference<T>
		...
		public Object p0, p1, p2, p3, p4, p5, p6, p7, p8, p9, pa, pb, pc, pd,
				pe;
	 ...
	 */
	private Object awaitFulfill(QNode pred, QNode s, Object e, int mode,
			long nanos) {
		if (mode == NOWAIT) {
			return null;
		}
		...
		for (;;) {
			...
			if (spins < 0) {
				...
				spins = h != null && h.next == s
						? (mode == TIMEOUT ? maxTimedSpins : maxUntimedSpins)
						: 0;
			}
			if (spins > 0) {
				--spins;
	...
	private QNode reclean() {
		...
		while ((pred = this.cleanMe.get()) != null) {
			...
			if (s != t) {
				QNode sn;
				if (s == null || s == pred || s.get() != s || (sn = s.next) == s
						|| pred.casNext(s, sn)) {
