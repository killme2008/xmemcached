...
public class MemcachedHandler extends HandlerAdapter {
  ...
  private static final int MAX_HEARTBEAT_THREADS =
      Integer.parseInt(System.getProperty("xmemcached.heartbeat.max_threads",
  ...
  private void checkHeartBeat(Session session) {
    if (this.enableHeartBeat) {
      ...
      if (this.client.getProtocol() == Protocol.Binary) {
        versionCommand = new BinaryVersionCommand(latch, session.getRemoteSocketAddress());

      } else {
        versionCommand = new TextVersionCommand(latch, session.getRemoteSocketAddress());
      }
  ...
  private static final int MAX_HEART_BEAT_FAIL_COUNT =
      Integer.parseInt(System.getProperty("xmemcached.heartbeat.max.fail.times", "3"));
  ...
  final static class CheckHeartResultThread implements Runnable {
    ...
    public void run() {
      try {
        AtomicInteger heartBeatFailCount =
            (AtomicInteger) this.session.getAttribute(HEART_BEAT_FAIL_COUNT_ATTR);
        if (heartBeatFailCount != null) {
          if (!this.versionCommand.getLatch().await(2000, TimeUnit.MILLISECONDS)) {
          ...
          if (heartBeatFailCount.get() > MAX_HEART_BEAT_FAIL_COUNT) {
            log.warn("Session(" + SystemUtils.getRawAddress(this.session.getRemoteSocketAddress())
                + ":" + this.session.getRemoteSocketAddress().getPort() + ") heartbeat fail "
                + heartBeatFailCount.get() + " times,close session and try to heal it");
  ...
  protected void reconnect(MemcachedTCPSession session) {
    if (!this.client.isShutdown()) {
      ...
      this.client.getConnector().addToWatingQueue(
          new ReconnectRequest(inetSocketAddressWrapper, 0, this.client.getHealSessionInterval()));
    }
  }
  ...
  final long HEARTBEAT_PERIOD =
      Long.parseLong(System.getProperty("xmemcached.heartbeat.period", "5000"));
  ...
  public void start() {
    ...
    this.heartBeatThreadPool = new ThreadPoolExecutor(1, MAX_HEARTBEAT_THREADS, keepAliveTime,
