...
public class BinarySetMultiCommand extends BaseBinaryCommand implements MergeCommandsAware {
  ...
  protected boolean finish() {
    final Boolean rt = ((Map<Integer, Boolean>) this.result).get(this.responseOpaque);
    Map<Object, Command> mergetCommands = getMergeCommands();
    if (mergetCommands != null) {
      final BinaryStoreCommand command =
          (BinaryStoreCommand) mergetCommands.remove(this.responseOpaque);
      if (command != null) {
  ...
  protected boolean readOpaque(ByteBuffer buffer) {
    ...
    if (cmd == null) {
      ...
      if (this.responseStatus == ResponseStatus.NO_ERROR) {
        ((Map<Integer, Boolean>) this.result).put(responseOpaque, Boolean.TRUE);
      } else {
        ((Map<Integer, Boolean>) this.result).put(responseOpaque, Boolean.FALSE);
      }
      return true;
    }
