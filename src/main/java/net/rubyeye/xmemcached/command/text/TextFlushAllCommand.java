...
public class TextFlushAllCommand extends Command {
  ...
  public TextFlushAllCommand(final CountDownLatch latch, int delay, boolean noreply) {
    ...
    if (isNoreply()) {
      if (this.exptime <= 0) {
        ...
        this.ioBuffer = IoBuffer
            .allocate("flush_all".length() + 2 + delayBytes.length + Constants.NO_REPLY.length + 2);
