...
public class TextTouchCommand extends Command {
  ...
  public TextTouchCommand(String key, byte[] keyBytes, CommandType cmdType, CountDownLatch latch,
  ...
  public final void encode() {
    ...
    int size =
        7 + this.keyBytes.length + ByteUtils.stringSize(this.expTime) + Constants.CRLF.length;
    if (isNoreply()) {
      size += 8;
    }
