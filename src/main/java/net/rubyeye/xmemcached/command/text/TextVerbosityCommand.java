...
public class TextVerbosityCommand extends VerbosityCommand {
  ...
  @Override
  public void encode() {
    final byte[] levelBytes = ByteUtils.getBytes(String.valueOf(this.level));
    if (isNoreply()) {
      this.ioBuffer =
          IoBuffer.allocate(4 + VERBOSITY.length() + levelBytes.length + Constants.NO_REPLY.length);
      ByteUtils.setArguments(this.ioBuffer, VERBOSITY, levelBytes, Constants.NO_REPLY);
    } else {
      this.ioBuffer = IoBuffer.allocate(2 + 1 + VERBOSITY.length() + levelBytes.length);
