...
public abstract class Command implements WriteMessage {
  ...
  public Command(final String key, final CommandType commandType, final CountDownLatch latch) {
    super();
    this.key = key;
    this.commandType = commandType;
    this.latch = latch;
  ...
  protected final boolean decodeError(String line) {
    if (line.startsWith("ERROR")) {
      ...
      String errorMsg = splits.length >= 2 ? splits[1] : "Unknow command " + getCommandType();
      ...
      setException(new MemcachedClientException(getErrorMsg(line, "Unknown Client Error")));
      this.countDownLatch();
      return true;
    } else if (line.startsWith("SERVER_ERROR")) {
      setException(new MemcachedServerException(getErrorMsg(line, "Unknown Server Error")));
      this.countDownLatch();
      return true;
    } else {
      throw new MemcachedDecodeException(
