...
public interface MemcachedClient {
  ...
  /**
   * Default thread number for reading nio's receive buffer and dispatch commands.Recommend users to
   * set it equal or less to the memcached server's number on linux platform,keep default on
   * windows.Default is 0.
   */
  ...
  /**
   * With java nio,there is only one connection to a memcached.In a high concurrent enviroment,you
   * may want to pool memcached clients.But a xmemcached client has to start a reactor thread and
   * some thread pools,if you create too many clients,the cost is very large. Xmemcached supports
   * connection pool instreadof client pool.you can create more connections to one or more memcached
  ...
  /**
   * Default session idle timeout,if session is idle,xmemcached will do a heartbeat action to check
  ...
  int DYNAMIC_MAX_QUEUED_NOPS =
      (int) (MAX_QUEUED_NOPS * (Runtime.getRuntime().maxMemory() / 1024.0 / 1024.0 / 1024.0));

  /**
   * Default max queued noreply operations number.It is calcuated dynamically based on your jvm
   * maximum memory.
   *
   * @since 1.3.8
   */
  public static final int DEFAULT_MAX_QUEUED_NOPS =
      DYNAMIC_MAX_QUEUED_NOPS > MAX_QUEUED_NOPS ? MAX_QUEUED_NOPS : DYNAMIC_MAX_QUEUED_NOPS;
  ...
  public <T> Map<String, GetsResponse<T>> gets(final Collection<String> keyCollections,
      final long opTime, final Transcoder<T> transcoder)
      throws TimeoutException, InterruptedException, MemcachedException;
   ...
   * @throws MemcachedException
   */
  public <T> void setWithNoReply(final String key, final int exp, final T value,
   ...
   * @throws MemcachedException
   */
  public <T> void addWithNoReply(final String key, final int exp, final T value,
      final Transcoder<T> transcoder) throws InterruptedException, MemcachedException;
   ...
   * @throws MemcachedException
   */
  public <T> void replaceWithNoReply(final String key, final int exp, final T value,
      final Transcoder<T> transcoder) throws InterruptedException, MemcachedException;
   ...
   * @throws MemcachedException
   */
  public boolean cas(final String key, final int exp, final Object value, final long timeout,
   ...
   * @throws MemcachedException
   */
  public <T> void casWithNoReply(final String key, GetsResponse<T> getsResponse,
      final CASOperation<T> operation)
      throws TimeoutException, InterruptedException, MemcachedException;
   ...
   *
   * @param <T>
   * @param keyExpMap A map,key is item's key,and value is a new expiration time for the item.
   * @param opTimeout operation timeout
   * @return
   * @throws TimeoutException
   * @throws InterruptedException
   ...
   * @throws MemcachedException
   */
  public long incr(final String key, final long delta, final long initValue, long timeout)
      throws TimeoutException, InterruptedException, MemcachedException;
   ...
   * @throws MemcachedException
   */
  public long decr(final String key, final long delta, long initValue, long timeout)
      throws TimeoutException, InterruptedException, MemcachedException;
