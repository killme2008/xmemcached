...
public class XMemcachedClient {
    ...
    private void connect(InetSocketAddress inetSocketAddress) {
        ...
        if (!connected) {
            this.connector.addToWatingQueue(new MemcachedConnector.ReconnectRequest(inetSocketAddress, 0));
    ...
    private Object get(final String key, long timeout, byte[] cmdBytes,
        ...
        final IoBuffer buffer = this.byteBufferAllocator.allocate(cmdBytes.length + ByteUtils.CRLF.length + 1 + keyBytes.length);
    ...
    public Map<String, Object> get(Collection<String> keyCollections)
        ...
        return (Map<String, Object>) get(keyCollections,
                lazy * DEFAULT_OP_TIMEOUT, ByteUtils.GET,
    ...
    public Map<String, GetsResponse> gets(Collection<String> keyCollections)
        ...
        long lazy = keyCollections.size() / 1000 > 0 ? (keyCollections.size() / 1000)
                : 1;
        lazy = lazy > 3 ? 3 : lazy; // 最高3秒
        return (Map<String, GetsResponse>) get(keyCollections, lazy * DEFAULT_OP_TIMEOUT, ByteUtils.GETS,
    ...
    public boolean delete(final String key, final int time)
        ...
        final IoBuffer buffer = this.byteBufferAllocator.allocate(ByteUtils.DELETE.length + 2 + keyBytes.length + timeBytes.length + ByteUtils.CRLF.length);
    ...
    private int sendIncrOrDecrCommand(final String key, final int num,
        ...
        byte[] keyBytes = ByteUtils.getBytes(key);
        final IoBuffer buffer = this.byteBufferAllocator.allocate(cmd.length() + 2 + key.length() + numBytes.length + ByteUtils.CRLF.length);
