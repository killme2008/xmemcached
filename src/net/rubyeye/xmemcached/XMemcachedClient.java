...
public final class XMemcachedClient {
     ...
     * @param optimiezeGet
     */
    public final void setOptimiezeGet(boolean optimiezeGet) {
    ...
    private void connect(InetSocketAddress inetSocketAddress) {
        ...
        try {
            ...
            if (!future.isDone() && !future.get(DEFAULT_CONNECT_TIMEOUT,
                    TimeUnit.MILLISECONDS)) {
                log.error("connect to " + inetSocketAddress.getHostName() + ":" + inetSocketAddress.getPort() + " fail");
            } else {
                connected = true;
            }
            ...
            log.error("connect to " + inetSocketAddress.getHostName() + ":" + inetSocketAddress.getPort() + " error", e);
        } catch (TimeoutException e) {
            if (future != null) {
                future.cancel(true);
            }
            log.error("connect to " + inetSocketAddress.getHostName() + ":" + inetSocketAddress.getPort() + " timeout", e);
        } catch (Exception e) {
            if (future != null) {
                future.cancel(true);
            }
     ...
     */
    @SuppressWarnings("unchecked")
    public final <T> T get(final String key, long timeout, Transcoder<T> transcoder)
    ...
    @SuppressWarnings("unchecked")
    public final Object get(final String key, long timeout) throws TimeoutException,
            InterruptedException, MemcachedException {
        return get(key, timeout, this.transcoder);
    }

    public final <T> T get(final String key, Transcoder<T> transcoder)
            throws TimeoutException, InterruptedException, MemcachedException {
     ...
     */
    @SuppressWarnings("unchecked")
    public final <T> GetsResponse<T> gets(final String key, long timeout,
    ...
    public final <T> GetsResponse<T> gets(final String key) throws TimeoutException,
            InterruptedException, MemcachedException {
    ...
    @SuppressWarnings("unchecked")
    public final <T> GetsResponse<T> gets(final String key, Transcoder transcoder)
            throws TimeoutException, InterruptedException, MemcachedException {
    ...
    @SuppressWarnings("unchecked")
    public final Map<String, GetsResponse> gets(Collection<String> keyCollections)
            throws TimeoutException, InterruptedException, MemcachedException {
        // 超时时间加倍
        long lazy = keyCollections.size() / 1000 > 0 ? (keyCollections.size() / 1000)
                : 1;
    ...
    public final boolean set(final String key, final int exp, Object value)
            throws TimeoutException, InterruptedException, MemcachedException {
    ...
    public final boolean add(final String key, final int exp, Object value)
            throws TimeoutException, InterruptedException, MemcachedException {
    ...
    public final boolean replace(final String key, final int exp, Object value)
            throws TimeoutException, InterruptedException, MemcachedException {
     ...
     * @throws MemcachedException
     */
    public final boolean append(final String key, Object value)
            throws TimeoutException, InterruptedException, MemcachedException {
     ...
     * @throws MemcachedException
     */
    public final boolean prepend(final String key, Object value)
            throws TimeoutException, InterruptedException, MemcachedException {
    ...
    public final boolean cas(final String key, final int exp, Object value, long cas)
            throws TimeoutException, InterruptedException, MemcachedException {
        return cas(key, exp, value, DEFAULT_OP_TIMEOUT, cas);
    }

    @SuppressWarnings("unchecked")
    public final boolean cas(final String key, final int exp, Object value,
