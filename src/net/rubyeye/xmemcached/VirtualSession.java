...
public class VirtualSession implements Session {
    ...
    public VirtualSession(int maxSize, Transcoder transcoder) {
        ...
        closed = false;
        this.cache = Collections.synchronizedMap(new LRUMap<String, CachedData>(this.maxSize));
        ...
        switch (command.getCommandType()) {
            ...
            case APPEND:
                synchronized (cache) {
                    if (cache.get(key) != null) {
                        ...
                        System.arraycopy(newData, 0, data, oldData.length, newData.length);
                        cache.put(key, new CachedData(existsData.getFlags(), data, data.length, -1));
                        command.setResult(Boolean.TRUE);
                    } else {
                        command.setResult(Boolean.FALSE);
                    }
            ...
            case PREPEND:
                synchronized (cache) {
                    if (cache.get(key) != null) {
                        ...
                        System.arraycopy(oldData, 0, data, newData.length, oldData.length);
                        cache.put(key, new CachedData(existsData.getFlags(), data, data.length, -1));
                        command.setResult(Boolean.TRUE);
                    } else {
                        command.setResult(Boolean.FALSE);
            ...
            case INCR:
                synchronized (cache) {
                    if (cache.get(key) != null) {
                        CachedData existsData = this.cache.get(key);
                        Integer count = Integer.parseInt((String) this.transcoder.decode(existsData));
                        count = command.getCommandType() == Command.CommandType.INCR ? count + 1 : count - 1;
                        CachedData newData = this.transcoder.encode(String.valueOf(count));
            ...
            case DELETE:
                ...
                break;
            default:
                throw new UnsupportedOperationException("Unsupported operation for virtual session:" + command.getCommandType().name());
        }
        return true;
    }
