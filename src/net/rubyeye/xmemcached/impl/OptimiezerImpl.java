 ...
 * @author dennis
 */
public class OptimiezerImpl implements Optimiezer {
    ...
    public OptimiezerImpl(BufferAllocator bufferAllocator) {
    ...
    public Command optimieze(final Command currentCommand, final Queue writeQueue, final BlockingQueue<Command> executingCmds, int sendBufferSize) {
        ...
        if (optimiezeCommand.getCommandType() == Command.CommandType.GET_ONE) {
            ...
            if (optimiezeGet) {
                writeQueue.remove();
                optimiezeCommand = optimizeGet(optimiezeCommand, writeQueue, executingCmds, mergeCommands);
                ((Deque) writeQueue).addFirst(optimiezeCommand); // 加入队首

            }
        }
        if (optimiezeMergeBuffer && optimiezeCommand.getIoBuffer().getByteBuffer().remaining() < sendBufferSize) {
            writeQueue.remove();
            optimiezeCommand = optimiezeBuffer(optimiezeCommand, writeQueue, executingCmds, sendBufferSize);
            ((Deque) writeQueue).addFirst(optimiezeCommand);// 加入队首
    ...
    private Command newMergedCommand(final List<Command> mergeCommands,
        ...
        final IoBuffer buffer = bufferAllocator.allocate(ByteUtils.GET.length + ByteUtils.CRLF.length + 1 + keyBytes.length);
        ByteUtils.setArguments(buffer, ByteUtils.GET, keyBytes);
        buffer.flip();
        Command cmd = new Command(key.toString(), Command.CommandType.GET_ONE, null) {
