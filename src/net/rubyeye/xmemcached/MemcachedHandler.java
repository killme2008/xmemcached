...
public class MemcachedHandler extends HandlerAdapter<Command> implements
     ...
     * @return
     */
    private boolean notifyBoolean(MemcachedTCPSession session, Boolean result, Command.CommandType expectedCmdType, Command.CommandType... otherExpectedCmdType) {
        final Command executingCmd = session.getCurrentExecutingCommand();
        boolean isExpected = executingCmd.getCommandType().equals(expectedCmdType);
        if (!isExpected) {
            for (Command.CommandType cmdType : otherExpectedCmdType) {
                if (cmdType.equals(executingCmd.getCommandType())) {
                    isExpected = true;
                    break;
                }
            }
            ...
            switch (session.status) {
                case NULL:
                    ...
                    if (session.currentLine.startsWith("VALUE")) {
                        ...
                        return notifyBoolean(session, Boolean.TRUE,Command.CommandType.SET,Command.CommandType.CAS,Command.CommandType.ADD,Command.CommandType.REPLACE);
                    } else if (session.currentLine.equals("DELETED")) {
                        return notifyBoolean(session, Boolean.TRUE,Command.CommandType.DELETE);
                    } else if (session.currentLine.equals("END")) {
                        return parseEndCommand(session);
                    } else if (session.currentLine.equals("EXISTS")) {
                        return notifyBoolean(session, Boolean.FALSE,Command.CommandType.CAS);
                    } else if (session.currentLine.equals("NOT_STORED")) {
                        return notifyBoolean(session, Boolean.FALSE,Command.CommandType.ADD,Command.CommandType.REPLACE);
                    } else if (session.currentLine.equals("NOT_FOUND")) {
                        return notifyBoolean(session, Boolean.FALSE,Command.CommandType.DELETE,Command.CommandType.CAS,Command.CommandType.INCR,Command.CommandType.DECR);
        ...
        while (true) {
            ...
            if (session.currentLine.equals("END")) {
                ...
                if (executingCommand.getCommandType().equals(
                        Command.CommandType.GET_MANY) || executingCommand.getCommandType().equals(
    ...
    private void processGetOneCommand(Session session,
        ...
        if(!executingCmd.getCommandType().equals(Command.CommandType.GET_ONE)&&
                !executingCmd.getCommandType().equals(Command.CommandType.GETS_ONE)){
            session.close();
            return;
        }
    ...
    private void processGetManyCommand(Session session,
            Map<String, CachedData> values, Command executingCmd) {
        if(!executingCmd.getCommandType().equals(Command.CommandType.GET_MANY)&&
                !executingCmd.getCommandType().equals(Command.CommandType.GETS_MANY)){
            session.close();
            return;
        }
        // 合并结果
