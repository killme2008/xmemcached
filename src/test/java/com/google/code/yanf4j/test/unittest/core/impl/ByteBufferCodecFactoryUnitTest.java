...
public class ByteBufferCodecFactoryUnitTest {
	...
	public void testEncodeNormal() throws Exception {
		...
		IoBuffer buffer = encoder
				.encode(IoBuffer.wrap("hello".getBytes("utf-8")), null);
	...
	public void testEncodeEmpty() throws Exception {
		...
		Assert.assertEquals(IoBuffer.allocate(0),
				encoder.encode(IoBuffer.allocate(0), null));
	}
	...
	public void decodeNormal() throws Exception {
		...
		IoBuffer decodeBuffer = (IoBuffer) this.codecFactory.getDecoder()
				.decode(buffer, null);
		Assert.assertEquals(IoBuffer.wrap("hello".getBytes("utf-8")),
				decodeBuffer);
	}
	...
	public void decodeEmpty() throws Exception {
		...
		Assert.assertEquals(IoBuffer.allocate(0), this.codecFactory.getDecoder()
				.decode(IoBuffer.allocate(0), null));
	}
