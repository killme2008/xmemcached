...
public class FutureImplUnitTest {
	...
	private static final Logger log = LoggerFactory
			.getLogger(FutureImplUnitTest.class);
	...
	private static final class NotifyFutureRunner implements Runnable {
		...
		public NotifyFutureRunner(FutureImpl<Boolean> future, long sleepTime,
				Throwable throwable) {
			super();
			this.future = future;
	...
	public void testGetException() throws Exception {
		FutureImpl<Boolean> future = new FutureImpl<Boolean>();
		new Thread(
				new NotifyFutureRunner(future, 2000, new IOException("hello")))
						.start();
		try {
			future.get();
			Assert.fail();
