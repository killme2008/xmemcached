...
public class SelectorManagerUnitTest {
	...
	public void testRegisterOpenChannel() throws Exception {
		...
		Reactor reactor = this.selectorManager.registerChannel(channel, 1,
				"hello");
	...
	public void testRegisterOpenSession() throws Exception {
		...
		EasyMock.expect(session.getAttribute(SelectorManager.REACTOR_ATTRIBUTE))
				.andReturn(null);
		EasyMock.expect(session.setAttributeIfAbsent(
				SelectorManager.REACTOR_ATTRIBUTE, nextReactor))
				.andReturn(null);
