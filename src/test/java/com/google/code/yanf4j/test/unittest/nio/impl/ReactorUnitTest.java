...
public class ReactorUnitTest {
	...
	public void setUp() throws Exception {
		...
		this.selectorManager = new SelectorManager(1, controller,
				configuration);
	...
	public void testRegisterOpenChannel() throws Exception {
		...
		Assert.assertSame(this.reactor.getSelector(),
				channel.selectionKey.selector);
	...
	private void addTimeoutKey(IMocksControl mocksControl,
	...
	private void addIdleKey(IMocksControl mocksControl,
			Set<SelectionKey> allKeys) {
		MockSelectionKey key = new MockSelectionKey();
		NioSession session = mocksControl.createMock(NioSession.class);
		key.attach(session);
	...
	private void addReadableKey(Set<SelectionKey> keySet, IMocksControl control,
			Selector selector) {
		MockSelectionKey key = new MockSelectionKey();
	...
	private void addWritableKey(Set<SelectionKey> keySet, IMocksControl control,
			Selector selector) {
		MockSelectionKey key = new MockSelectionKey();
