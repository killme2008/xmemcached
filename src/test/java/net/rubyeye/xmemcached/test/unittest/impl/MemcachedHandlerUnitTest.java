...
public class MemcachedHandlerUnitTest {
  ...
  public void setUp() {
    ...
    this.memcachedClient = this.mocksControl.createMock(MemcachedClient.class);
  ...
  public void testOnMessageSent_TextCommand() {
    Command cmd = new TextVersionCommand(new CountDownLatch(1), new InetSocketAddress(8080));
  ...
  public void testOnMessageSent_TextCommand_NoReply() {
    TextStoreCommand cmd =
        new TextStoreCommand(null, null, CommandType.SET, null, 1, 1, "hello", true, null);
  ...
  public void testOnMessageSent_BinaryCommand_NoReply() {
    BinaryStoreCommand cmd =
        new BinaryStoreCommand(null, null, CommandType.ADD, null, 1, 1, "hello", true, null);
    Assert.assertEquals("hello", cmd.getValue());
    this.mocksControl.replay();
    this.handler.onMessageSent(this.session, cmd);
    this.mocksControl.verify();
