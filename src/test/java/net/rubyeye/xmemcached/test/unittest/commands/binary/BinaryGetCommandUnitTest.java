...
public class BinaryGetCommandUnitTest extends BaseBinaryCommandUnitTest {
  ...
  public void testGetEncodeAndDecode() {
    ...
    ByteBuffer buffer = constructResponse(OpCode.GET.fieldValue(), (short) 0, (byte) 0x04, (byte) 0,
        (short) 0, 0x00000009, 0, 1L, this.transcoderUtils.encodeInt(0), null, "world".getBytes());

    assertEquals(33, buffer.capacity());
    assertTrue(command.decode(null, buffer));
    assertEquals("world", this.transcoder.decode((CachedData) command.getResult()));
    assertEquals(0, buffer.remaining());

    buffer = constructResponse(OpCode.GET.fieldValue(), (short) 0, (byte) 0x04, (byte) 0, (short) 0,
        0x00000004, 0, 1L, this.transcoderUtils.encodeInt(0), null, null);
    assertEquals(28, buffer.capacity());
    command = this.commandFactory.createGetCommand(this.key, this.keyBytes, CommandType.GET_ONE,
        this.transcoder);
    assertTrue(command.decode(null, buffer));
