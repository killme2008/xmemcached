...
public class TextCommandFactoryTest extends TestCase {
  ...
  public void testCreateDeleteCommand() {
    ...
    int time = 10;
    Command deleteCmd = this.commandFactory.createDeleteCommand("test", keyBytes, time, 0, false);
    deleteCmd.encode();
  ...
  public void testCreateStoreCommand() {
    ...
    String expectedStr = "set test " + StringTranscoder.STRING_FLAG + " 0 4\r\ntest\r\n";
    assertEquals(expectedStr, commandStr);
  }
  ...
  public void testCreateGetCommand() {
    String key = "test";
    byte[] keyBytes = ByteUtils.getBytes(key);
    Command getCmd = this.commandFactory.createGetCommand(key, keyBytes, CommandType.GET_ONE, null);
    getCmd.encode();
  ...
  public void testCreateIncrDecrCommand() {
    ...
    byte[] keyBytes = ByteUtils.getBytes(key);
    int num = 10;
    Command inCr = this.commandFactory.createIncrDecrCommand(key, keyBytes, num, 0, 0,
  ...
  public void testCreateGetMultiCommand() {
    ...
    Command cmd = this.commandFactory.createGetMultiCommand(keys, null, CommandType.GET_MANY, null);
    cmd.encode();
