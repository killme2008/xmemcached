...
public class TextVerbositylCommandUnitTest extends BaseTextCommandUnitTest {
  public void testEncode() {
    Command command = this.commandFactory.createVerbosityCommand(new CountDownLatch(1), 1, false);
    assertNull(command.getIoBuffer());
    command.encode();
    assertEquals("verbosity 1\r\n", new String(command.getIoBuffer().buf().array()));

    command = this.commandFactory.createVerbosityCommand(new CountDownLatch(1), 1, true);
    command.encode();
  ...
  public void testDecode() {
    Command command = this.commandFactory.createVerbosityCommand(new CountDownLatch(1), 0, false);
