...
public class BinaryGetMultiCommandUnitTest extends BaseBinaryCommandUnitTest {
  ...
  public void testEncodeDecode() {
    ...
    Command command = this.commandFactory.createGetMultiCommand(keys, new CountDownLatch(1),
        CommandType.GET_MANY, this.transcoder);
    command.encode();
    ...
    for (int i = 0; i < 9; i++) {
      ...
      if (i % 2 == 0) {
        // value==key
        buffers[i] = constructResponse(OpCode.GET_KEY_QUIETLY.fieldValue(), (short) 1, (byte) 0x04,
            (byte) 0, (short) 0, 6, 0, 1L, flagBytes, keyBytes, keyBytes);
      } else {
        // key not found
        buffers[i] = constructResponse(OpCode.GET_KEY_QUIETLY.fieldValue(), (short) 1, (byte) 0x04,
            (byte) 0, (short) 0x0001, 5, 0, 1L, flagBytes, keyBytes, null);
      }
    ...
    byte[] keyBytes = ByteUtils.getBytes(key);
    buffers[9] = constructResponse(OpCode.GET_KEY.fieldValue(), (short) 1, (byte) 0x04, (byte) 0,
