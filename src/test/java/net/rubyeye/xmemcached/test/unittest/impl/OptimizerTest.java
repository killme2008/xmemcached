...
public class OptimizerTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    for (int i = 0; i < 10; i++) {
      Command cmd = this.commandFactory.createGetCommand(String.valueOf(i),
          String.valueOf(i).getBytes(), CommandType.GET_ONE, null);
      cmd.encode();
  ...
  public void testOptimiezeSetLimitBuffer() {
    ...
    int oneBufferSize = 0;
    for (int i = 0; i < 10; i++) {
      Command cmd = this.commandFactory.createSetCommand(String.valueOf(i),
          String.valueOf(i).getBytes(), 0, i, false, transcoder);
      cmd.encode();
      this.writeQueue.add(cmd);
    ...
    assertEquals(optimiezedCommand.getMergeCount(), Math.round((double) limit / oneBufferSize));
  }
  ...
  public void testOptimiezeSetAllBuffers() {
    ...
    BinarySetMultiCommand optimiezedCommand = (BinarySetMultiCommand) this.optimiezer
        .optimiezeSet(this.writeQueue, this.executingCmds, this.currentCmd, Integer.MAX_VALUE);
  ...
  public void testOptimiezeGet() {
    ...
    TextGetOneCommand optimiezeCommand = (TextGetOneCommand) this.optimiezer
        .optimiezeGet(this.writeQueue, this.executingCmds, this.currentCmd);
  ...
  public void testOptimiezeGetWithSameKey() {
    ...
    for (int i = 0; i < 10; i++) {
      Command cmd = this.commandFactory.createGetCommand(String.valueOf(0),
          String.valueOf(0).getBytes(), CommandType.GET_ONE, null);
      cmd.encode();
      this.writeQueue.add(cmd);
      cmd.setWriteFuture(new FutureImpl<Boolean>());
      localQueue.add(cmd);
    ...
    assertEquals("10", transcoder.decode((CachedData) this.currentCmd.getResult()));
  ...
  public void testMergeDifferenceCommands() {
    ...
    for (int i = 5; i < 10; i++) {
      Command cmd = this.commandFactory.createDeleteCommand(String.valueOf(i),
          String.valueOf(i).getBytes(), 0, 0, false);
      this.writeQueue.add(cmd);
      cmd.setWriteFuture(new FutureImpl<Boolean>());
    }
  ...
  public void testMergeGetCommandsWithEmptyWriteQueue() {
    ...
    Command optimiezeCommand =
        this.optimiezer.optimiezeGet(this.writeQueue, this.executingCmds, this.currentCmd);
    optimiezeCommand.encode();
  ...
  public void testMergeLimitBuffer() {
    ...
    Command optimiezeCommand = this.optimiezer.optimiezeMergeBuffer(this.currentCmd,
        this.writeQueue, this.executingCmds, 54);
    assertNotSame(this.currentCmd, optimiezeCommand);
    ByteBuffer mergeBuffer = optimiezeCommand.getIoBuffer().buf();
    assertEquals(0, this.writeQueue.size());
    assertSame(CommandType.GET_ONE, optimiezeCommand.getCommandType());
    assertEquals("get 0\r\nget 1 2 3 4 5 6 7 8 9\r\n", new String(mergeBuffer.array())); // current
                                                                                         // command
                                                                                         // at last
  }
  ...
  public void testMergeAllBuffer() {
    ...
    this.optimiezer.setOptimizeMergeBuffer(true);
    Command optimiezeCommand = this.optimiezer.optimiezeMergeBuffer(this.currentCmd,
        this.writeQueue, this.executingCmds, 100);
    ByteBuffer mergeBuffer = optimiezeCommand.getIoBuffer().buf();
    assertNotSame(this.currentCmd, optimiezeCommand);
    assertTrue(mergeBuffer.remaining() < 100);
    assertEquals(0, this.writeQueue.size());
  ...
  public void testOptimieze() {
    this.optimiezer.setOptimizeMergeBuffer(true);
    for (int i = 0; i < 10; i++) {
      Command deleteCommand = this.commandFactory.createDeleteCommand(String.valueOf(i),
          String.valueOf(i).getBytes(), 0, 0, false);
    ...
    Command optimiezeCommand =
        this.optimiezer.optimize(this.currentCmd, this.writeQueue, this.executingCmds, 16 * 1024);
    ByteBuffer mergeBuffer = optimiezeCommand.getIoBuffer().buf();
