...
public class SessionLocatorTest extends TestCase {
  ...
  public void testArraySessionLocator() {
    ...
    // use next
    assertSame(sessions.get(oldIndex + 1), sessionLocator.getSessionByKey(key));
  ...
  public void testKetamaMemcachedSessionLocator() {
    sessionLocator = new KetamaMemcachedSessionLocator(HashAlgorithm.NATIVE_HASH);
    List<Session> sessions = new ArrayList<Session>();
    for (int i = 8080; i < 8100; i++) {
      sessions.add(new MockSession(i));
    }
    ...
    for (int i = 1; i <= 10; i++) {
      ...
      assertSame(((KetamaMemcachedSessionLocator) sessionLocator).getSessionByHash(key.hashCode()),
          session);
