...
public class KestrelClientIT extends TestCase {
  ...
  private MemcachedClientBuilder newBuilder() {
    MemcachedClientBuilder builder = new XMemcachedClientBuilder(
        AddrUtil.getAddresses(this.properties.getProperty("test.kestrel.servers")));
  ...
  public void testSetAndGetObject() throws Exception {
    ...
    Map<String, String> mapFromMQ = (Map<String, String>) this.memcachedClient.get("queue1");
    ...
    List<UserDefinedClass> userDefinedClassListFromMQ =
        (List<UserDefinedClass>) this.memcachedClient.get("queue1");
  ...
  public void testBlockingFetch() throws Exception {
    ...
    Assert.assertTrue(this.memcachedClient.set("queue1", 0, "hello world"));
    Assert.assertEquals("hello world", this.memcachedClient.get("queue1/t=1000"));
  ...
  public void testReliableFetch() throws Exception {
    Assert.assertTrue(this.memcachedClient.set("queue1", 0, "hello world"));
    Assert.assertEquals("hello world", this.memcachedClient.get("queue1/open"));
    // close connection
    this.memcachedClient.shutdown();
    // still can fetch it
  ...
  public void testPerformance() throws Exception {
    ...
    System.out.println("push 10000 message:" + (System.currentTimeMillis() - start) + "ms");

    start = System.currentTimeMillis();
    for (int i = 0; i < 10000; i++) {
      this.memcachedClient.get("queue1");
    }
    System.out.println("fetch 10000 message:" + (System.currentTimeMillis() - start) + "ms");
  }
  ...
  class AccessThread extends Thread {
    ...
    public void run() {
      try {
        ...
        for (int i = 0; i < 10000; i++) {
          KestrelClientIT.this.memcachedClient.set("queue1", 0, "hello");
        }
        for (int i = 0; i < 10000; i++) {
