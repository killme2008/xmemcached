...
public class StandardHashMemcachedClientIT extends XMemcachedClientIT {
  ...
  public void testLargeObject() throws Exception {
    ...
    SerializingTranscoder transcoder = new SerializingTranscoder(megabyte_plus1 * 2); // something
                                                                                      // bigger than
                                                                                      // memcached
                                                                                      // daemon
                                                                                      // max value size.
    ...
    for (int i = 0; i < 5; i++) {
      try {
        ...
        this.memcachedClient.set(KEY_LARGE_OBJECT, 60, largeObject, transcoder);
        fail();

      } catch (MemcachedServerException exception) {
        Assert.assertTrue(exception.getMessage().contains("object too large for cache"));
      }
    }
  ...
  public void testStoreNoReply() throws Exception {
    ...
    memcachedClient.setWithNoReply("name", 1, "dennis", new StringTranscoder());
    assertEquals("dennis", memcachedClient.get("name"));
    Thread.sleep(2000);
    assertNull(memcachedClient.get("name"));

    memcachedClient.setWithNoReply("name", 0, "dennis", new StringTranscoder());
    ...
    memcachedClient.addWithNoReply("name", 0, "test", new StringTranscoder());
    assertEquals("hello dennis zhuang", memcachedClient.get("name"));
    memcachedClient.replaceWithNoReply("name", 0, "test", new StringTranscoder());
    ...
    memcachedClient.casWithNoReply("a", 0, getsResponse, new CASOperation<Integer>() {
    ...
    memcachedClient.casWithNoReply("a", getsResponse, new CASOperation<Integer>() {
