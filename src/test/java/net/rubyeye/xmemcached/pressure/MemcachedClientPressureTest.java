...
public class MemcachedClientPressureTest {
  ...
  static class TestThread extends Thread {
    ...
    public TestThread(int repeat, int start, int size, MemcachedClient client,
        AtomicInteger failure, AtomicInteger success, CyclicBarrier barrier) {
      super();
      this.repeat = repeat;
      this.start = start;
      this.size = size;
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 0; i < threads; i++) {
      new TestThread(repeat, i * repeat * 2, size, client, failure, success, barrier).start();
    }
    new Thread() {
      public void run() {
        try {
    ...
    client.shutdown();
    System.out.println(
        "duration:" + secs + " seconds,tps:" + tps + " op/seconds,total:" + total + " ops");
